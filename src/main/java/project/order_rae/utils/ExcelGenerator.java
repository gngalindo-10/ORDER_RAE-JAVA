...
public class ExcelGenerator {
    ...
    private CellStyle createHeaderStyle(Workbook workbook) {
        ...
        public void generarExcelFidelizacion(List<Fidelizacion> datos, HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=fidelizacion_" + 
                LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss")) + ".xlsx");

        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Fidelización");
            Row header = sheet.createRow(0);
            String[] cols = {"ID", "Usuario", "Fecha Fidelización", "Puntos Acumulados", "Nivel Fidelización"};
            for (int i = 0; i < cols.length; i++) {
                header.createCell(i).setCellValue(cols[i]);
                header.getCell(i).setCellStyle(createHeaderStyle(workbook));
            }
            ...
            int rowIdx = 1;
            for (Fidelizacion f : datos) {
                Row row = sheet.createRow(rowIdx++);
                row.createCell(0).setCellValue(f.getId());
                
                String usuario = "";
                if (f.getUsuario() != null) {
                    usuario = f.getUsuario().getNombre() + " " + f.getUsuario().getApellidos();
                }
                row.createCell(1).setCellValue(usuario);
                
                row.createCell(2).setCellValue(f.getFechaFidelizacion() != null ? f.getFechaFidelizacion().toString() : "");
                row.createCell(3).setCellValue(f.getPuntosAcumulados() != null ? f.getPuntosAcumulados() : 0);
                row.createCell(4).setCellValue(f.getNivelFidelizacion() != null ? f.getNivelFidelizacion() : "");
            }
